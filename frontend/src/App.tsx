...
function App() {
  ...
  useEffect(() => {
    window.navigateToPayment = handleNavigateToPayment
    ...
    return () => {
      ...
      delete window.navigateToPayment
    }
  }, [])
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center">
                      <span className="text-white text-sm font-semibold">{user?.firstName?.[0] || user?.username?.[0] || 'U'}</span>
                    </div>
          ...
          <Tabs defaultValue="discover" value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="matches" className="p-6">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-light text-slate-800">Your Matches</h2>
                <span className="text-pink-500 text-sm font-medium">4 new matches</span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
                {profiles.map(profile => (
                  <div key={profile.id} className="bg-white/60 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-white/20 hover:scale-105 cursor-pointer" onClick={() => openProfileModal(profile)}>
                    <div className="h-36 overflow-hidden relative group">
                      ...
                      <div className="absolute top-1.5 right-1.5">
                        <div className="w-6 h-6 bg-gradient-to-r from-pink-500 to-violet-500 rounded-full flex items-center justify-center shadow-lg">
                          <Star className="h-3 w-3 text-white" fill="white" />
                        </div>
                      </div>
                      <div className="absolute bottom-1.5 left-1.5 right-1.5 text-white">
                        <h3 className="text-sm font-medium">{profile.name}, {profile.age}</h3>
                    ...
                    <div className="p-2.5 flex justify-between">
                      <Button size="sm" variant="ghost" className="w-[48%] h-8 text-xs text-slate-600 hover:text-slate-900 transition-colors">
                        Message
                      </Button>
                      <Button size="sm" className="w-[48%] h-8 text-xs bg-gradient-to-r from-pink-500 to-violet-500 hover:from-pink-600 hover:to-violet-600 text-white shadow hover:shadow-md transition-all duration-200">
                        View
                      </Button>
                    </div>
                  </div>
